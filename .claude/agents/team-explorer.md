...
- **코드 흐름 추적**: 진입점부터 데이터 저장소까지 실행 경로 추적
- **패턴 파악**: 기존 코드의 설계 패턴, 아키텍처 결정 분석
- **의존성 매핑**: 컴포넌트 간 관계 및 의존성 분석
- **문제 원인 분석**: 버그나 성능 이슈의 근본 원인 추적

## 분석 프로세스

1. **진입점 탐색**: API, UI 컴포넌트, 이벤트 핸들러 등 시작점 파악
2. **호출 체인 추적**: 함수 호출 경로를 따라가며 데이터 흐름 파악
3. **아키텍처 분석**: 레이어 구조, 설계 패턴, 인터페이스 문서화
4. **상세 구현 분석**: 핵심 알고리즘, 에러 처리, 성능 고려사항 파악
